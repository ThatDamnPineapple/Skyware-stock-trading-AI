        ...
        {
            {
                 if (Main.rand.Next(2) == 0)
                    {
                        ...
                        int proj = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, velocity.X, velocity.Y, ProjectileID.CrystalShard, 20, projectile.owner, 0, 0f);
                 ...
                 if (Main.rand.Next(2) == 0)
                {
                    ...
                    float rotation = (float)(Main.rand.Next(0, 0) * (Math.PI / 180));
                    Vector2 velocity = new Vector2((float)Math.Cos(rotation), (float)Math.Sin(rotation));
                    int proj = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, velocity.X, velocity.Y, ProjectileID.CrystalShard, projectile.damage / 4 * 3, projectile.owner, 0, 0f);
                    Main.projectile[proj].friendly = true;
                    Main.projectile[proj].hostile = false;
                    Main.projectile[proj].velocity *= 4f;
                }
