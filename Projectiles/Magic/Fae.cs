...
<<<<<<< HEAD
            ...
            {
                {
                    ...
                    Lighting.AddLight((int)(projectile.Center.X / 16f), (int)(projectile.Center.Y / 16f), 0.5f, 0.5f, 0.9f);
                    {
                        for (int i = 0; i < 10; i++)
                        {
                            ...
                            int num = Dust.NewDust(new Vector2(x, y), 1, 1, 242, 0f, 0f, 0, default(Color), 1f);
        ...
        public override void Kill(int timeLeft)
        {
            Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 14);
            {
                for (int i = 0; i < 40; i++)
                {
                    int num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 242, 0f, -2f, 0, default(Color), 2f);
                    Main.dust[num].noGravity = true;
                }
            }
        }
