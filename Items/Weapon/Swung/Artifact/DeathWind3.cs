...
namespace SpiritMod.Items.Weapon.Swung.Artifact
{
    public class DeathWind3 : ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            item.useStyle = 1;        
            item.knockBack = 6;
            item.value = Terraria.Item.sellPrice(0, 9, 0, 50);
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            if (player.altFunctionUse == 2)
            {
                ...
                Terraria.Projectile.NewProjectile(player.Center.X, player.Center.Y, speedX, 0, mod.ProjectileType("SoulNet1"), 0, 0, item.owner);
                return true;
            }
            else
            {
